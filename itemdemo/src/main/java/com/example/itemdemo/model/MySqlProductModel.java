    ...
    public List<Product> findAll() {
        ...
        try {
            ...
            while(resultSet.next()){
                ...
                Product product = new Product(id, name, price, thumbnail, createdAt, updatedAt, status);
                list.add(product);
            }
        } catch (SQLException e) {
            e.printStackTrace();
        ...
        try {
            ...
            if(resultSet.next()){
                ...
                LocalDateTime updatedAt =
                        LocalDateTime.ofInstant(resultSet.getTimestamp("updatedAt").toInstant(), ZoneId.systemDefault());
                int status = resultSet.getInt("status");
                product = new Product(id, name, price, thumbnail, createdAt, updatedAt, status);
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
        ...
        public Product update(String id, Product updateProduct) {
            try {
                ...
                String sqlQuery = "update products " +
                        "set id = ?, name = ?, price = ?, thumbnail = ?, createdAt = ?, updatedAt = ?, status = ? where id = ?";
                PreparedStatement preparedStatement = connection.prepareStatement(sqlQuery);
